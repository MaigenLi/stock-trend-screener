...
TURN_POWER    = 0.48  # 幂指数（由4个锚点拟合得出）
TURN_CAP_REF  = 100.0  # 参考市值（亿元）
TURN_BASE     = 5.0    # 参考市值对应的换手率(%)
TURN_FLOOR    = 1.5    # 换手率下限兜底
...
GAIN20_MIN    = 7.0  # 下调：目标股20日涨幅多在7~12%
GAIN20_MID    = 12.0  # 20日涨幅中限%%
GAIN20_MAX    = 120.0  # 上调：目标股20日涨幅可达135%
GAIN20_MID    = 12.0  # 20日涨幅中限%%
GAIN5_MIN     = -2.0  # 放宽：允许小幅回调（目标股5日涨幅多在-2~1%）
GAIN5_MID     = 5.0  # 5日涨幅中限%%
GAIN5_MAX     = 60.0   # 上调：目标股5日涨幅可达53%
MKT_CAP_MIN   = 30.0  # 流通市值下限（亿元）
GAIN_DAY_MIN  = -1.5  # 当日涨幅下限%%
GAIN_DAY_MAX  = 25.0   # 上调：允许涨停/接近涨停
MA_LEN        = 20    # 计算MA20需要
...
MA5_DAILY  = 0.0   # 允许MA5横走或微涨
MA10_DAILY = 0.0   # 进一步放宽：允许MA10横走
MA20_DAILY = 0.0   # 进一步放宽：允许MA20横走
CLOSE_NEAR_HIGH_RATIO = 0.95   # 当前收盘 > 20日最高收盘 × CLOSE_NEAR_HIGH_RATIO

# 均线斜率 / ATR波动率
SLOPE_MA5_ATR     = -0.15  # 下限：允许整理期/缓涨股（目标股MA5_atr多在-0.15~0.04）
SLOPE_MA10_ATR    = -0.15  # 下限：允许整理期
SLOPE_MA20_ATR    = -0.15  # 下限：允许整理期

SLOPE_MA5_ATR_MAX     = 1.80   # 上调：目标股MA5_atr可达1.6+
SLOPE_MA10_ATR_MAX    = 1.20   # 上调：目标股MA10_atr可达1.0+
SLOPE_MA20_ATR_MAX    = 1.20   # 上调：覆盖整理末期股
# 特殊通道的上限同步调整（外部引用）
LIMITUP_I_SLOPE_MA5   = 0.3   # ma5_slope
LIMITUP_I_DAILY_MA5   = 1.0   # ma5_daily  > 1.0%
LIMITUP_I_SLOPE_MA10   = 0.25   # ma10_slope
LIMITUP_I_SLOPE_MA20   = 0.18   # ma20_slope
...
选股条件（同时满足）：
  ...
  [5] 20日涨幅 {GAIN20_MID:.0f}%%~{GAIN20_MAX:.0f}%%
...
def _cond2a_slope_atr(idx, ma5_slope_atr, ma10_slope_atr, ma20_slope_atr):
    ...
    if not ok:
        msg = []
        if not ok5:  msg.append(f"MA5_atr={ma5_slope_atr[idx]:.3f}不在[{SLOPE_MA5_ATR},{SLOPE_MA5_ATR_MAX})")
        if not ok10: msg.append(f"MA10_atr={ma10_slope_atr[idx]:.3f}不在[{SLOPE_MA10_ATR},{SLOPE_MA10_ATR_MAX})")
        if not ok20: msg.append(f"MA20_atr={ma20_slope_atr[idx]:.3f}不在[{SLOPE_MA20_ATR},{SLOPE_MA20_ATR_MAX})")
        return False, " ".join(msg)
    return True, f"MA5_atr={ma5_slope_atr[idx]:.3f} MA10_atr={ma10_slope_atr[idx]:.3f} MA20_atr={ma20_slope_atr[idx]:.3f}"

def _cond2b_ma_daily(idx, ma5_daily, ma10_daily, ma20_daily):
    """②-2 MA5/MA10/MA20 日变 >= 阈值（均线向上推，目标股多为缓涨/横走）"""
    # 只要求 MA5 >= 0（允许横走），MA10/MA20 可小幅下降
    ok = ma5_daily >= MA5_DAILY and ma10_daily >= -0.5 and ma20_daily >= -0.5
    if not ok:
        return False, f"MA日变不满足：MA5={ma5_daily:.3f}% MA10={ma10_daily:.3f}% MA20={ma20_daily:.3f}%"
    return True, f"MA5日变={ma5_daily:.3f}% MA10日变={ma10_daily:.3f}% MA20日变={ma20_daily:.3f}%"
...
def _cond4_turnover(idx, turnover_avg, turnover_now, turnover_prev, gain_day, mktcap):
    ...
    if not ok:
        ok = (turnover_now > TURN_FLOOR and turnover_now > turnover_prev and gain_day >= 3)
        if ok:
            return True, f"换手豁免：当日{turnover_now:.1f}%放量且涨幅{gain_day:.1f}%>=3%"
        else:
...
def _cond5_gain20(idx, gain20, gain5):
    """⑤ 20日涨幅在合理区间（需配合5日涨幅确认）"""
    ok = GAIN20_MAX >= gain20 >= GAIN20_MID
    if not ok:
        ok = GAIN20_MAX >= gain20 >= GAIN20_MIN and gain5 >= GAIN5_MID
        if ok:
            return True, f"20日涨幅{gain20:+.1f}%（松弛区间，5日涨幅{gain5:+.1f}%>=GAIN5_MID确认）"
        else:
            return False, f"20日涨幅{gain20:+.1f}% 不在[{GAIN20_MIN},{GAIN20_MAX}]%或5日涨幅不足确认"
    return True, f"20日涨幅{gain20:+.1f}% ∈ [{GAIN20_MID},{GAIN20_MAX}]%"
...
def _cond_lim_i1(idx, close, ma5_slope_atr, ma5_daily_i1):
...
def _cond_lim_i2(idx, close, ma5_slope_atr, ma5_daily_i2):
...
def _check_limitup_conditions(df: pd.DataFrame, signal_date: str = None, code: str = None):
    ...
    ok_gain1, ok_slope1, ok_daily1, gain_i1 = _cond_lim_i1(i, close, ma5_slope_atr, ma5_daily_i1)
    lines.append(f"【前1天 {str(df.iloc[i-1]['date'])[:10]}】")
    lines.append(f"  涨幅={gain_i1:.2f}% < 6% → {'✓' if ok_gain1 else '✗'}")
    ...
    ok_gain2, ok_slope2, ok_daily2, gain_i2 = _cond_lim_i2(i, close, ma5_slope_atr, ma5_daily_i2)
    lines.append(f"【前2天 {str(df.iloc[i-2]['date'])[:10]}】")
    lines.append(f"  涨幅={gain_i2:.2f}% < 2% → {'✓' if ok_gain2 else '✗'}")
    ...
    ma5_daily_i1 = (ma5[i - 1] / ma5[i - 2] - 1) * 100 if i - 1 >= 1 else -999
    ok_gain1  = gain_i1 < 6.0
    ok_slope1 = LIMITUP_I1_SLOPE_MA5_U > ma5_slope_atr[i - 1]
    ok_daily1 = LIMITUP_I1_DAILY_U > ma5_daily_i1
    lines.append(f"  涨幅={gain_i1:.2f}% < 6% → {'✓' if ok_gain1 else '✗'}")
    lines.append(f"  ma5_slope_atr={ma5_slope_atr[i-1]:.3f} < {LIMITUP_I1_SLOPE_MA5_U} → {'✓' if ok_slope1 else '✗'}")
    lines.append(f"  ma5_daily={ma5_daily_i1:.3f}% < {LIMITUP_I1_DAILY_U}% → {'✓' if ok_daily1 else '✗'}")
    ...
    ma5_daily_i2 = (ma5[i - 2] / ma5[i - 3] - 1) * 100 if i - 2 >= 1 else -999
    ok_gain2  = gain_i2 < 2.0
    ok_slope2 = ma5_slope_atr[i - 2] < LIMITUP_I2_SLOPE_MA5_U
    ok_daily2 = ma5_daily_i2 < LIMITUP_I2_DAILY_U
    lines.append(f"  涨幅={gain_i2:.2f}% < 2% → {'✓' if ok_gain2 else '✗'}")
    lines.append(f"  ma5_slope_atr={ma5_slope_atr[i-2]:.3f} < {LIMITUP_I2_SLOPE_MA5_U} → {'✓' if ok_slope2 else '✗'}")
    lines.append(f"  ma5_daily={ma5_daily_i2:.3f}% < {LIMITUP_I2_DAILY_U}% → {'✓' if ok_daily2 else '✗'}")
...
def _check_ma_conditions(df: pd.DataFrame, signal_date: str = None):
    ...
    lines.append(f"②-1 {msg2a} → {'✓' if ok2a else '✗'}")

    ok2b, msg2b = _cond2b_ma_daily(i, ma5_d, ma10_d, ma20_d)
    lines.append(f"②-2 {msg2b} → {'✓' if ok2b else '✗'}")
    ...
    lines.append(f"\n  最终：{'✓ 通过全部条件' if (ok1 and ok2a and ok2b and ok3 and ok4 and ok5 and ok6 and ok7 and ok8) else '✗ 淘汰'}")
    return lines
...
def check_limitup_channel(df: pd.DataFrame, signal_date: str = None, code: str = None) -> dict | None:
    ...
    # 涨停价 = 前收盘 × (1 + limit_ratio)，四舍五入到 0.01
    limit_up_price = round(prev_close_i * (1 + limit_ratio), 2)
    recent_20_high = float(np.max(close[i - 19:i + 1])) if i >= 19 else float(np.max(close[:i + 1]))
    if not (abs(close[i] - limit_up_price) < 0.005 and close[i] >= recent_20_high * 0.9999):
        return None
    ...
    return {
        ...
        "ma10_slope_atr":     round(ma10_slope_atr[i], 3),
        "ma20_slope":     round(ma20_slope_atr[i], 3),
...
|  [3] 流通市值 >= {MKT_CAP_MIN:.0f}亿
|  [4] 近5日平均换手率 >= 自适应（幂律，市值越大要求越低）或 当日涨幅>=3% 并且 当日换手率 >= {TURN_BASE:.0f}% 并且量比前一天放大
...
def print_results(results, signal_date=None, weekday_cn=None):
    ...
    print(sep)
    print("%3s  %-8s  %-8s  %-10s  %-5s  %-6s  %-5s  %-5s %-5s %-5s" % ("#","代码","名称","日期","收盘","20日涨","均换手","换手今","量比", "市值(亿)"))
    print("-" * 95)
    for rank, r in enumerate(results, 1):
        tag = " 🔴特殊" if r.get("_limitup") else ""
        print("%3d  %-8s  %-8s  %-10s  %7.2f  %+7.1f%%  %7.2f%%  %6.2f%%  %5.2fx  %5.0f%s" % (
            rank, r["code"], r["name"], r["date"], r["close"],
            r["gain20"], r["turnover_avg"], r["turnover_today"], r["vol_ratio"], r["mktcap"], tag))
    print()
...
def print_detail(results: list):
    ...
    for rank, r in enumerate(results, 1):
        ...
        print(f"   ②-1 均线斜率/ATR波动率: {r['ma5_slope_atr']:.3f}%({'↑' if ok2a else '✗5'})  {r['ma10_slope_atr']:.3f}%({'↑' if ok2a else '✗10'})  {r['ma20_slope_atr']:.3f}%({'↑' if ok2a else '✗20'})  →  {'✓' if ok2a else '✗'}")
        print(f"   ②-2 MA5日变{r['ma5_daily']:+.3f}%({'↑' if ok2b else '✗<{MA5_DAILY}%'})  MA10日变{r['ma10_daily']:+.3f}%({'↑' if ok2b else '✗<{MA10_DAILY}%'})  MA20日变{r['ma20_daily']:+.3f}%({'↑' if ok2b else '✗<{MA20_DAILY}%'})")
        mc_d = r.get('mktcap', 0)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f"条件选股器（同时满足以下全部条件）\n\n"
                    ...
                    f"条件⑤：20日涨幅 {int(GAIN20_MID)}%~{int(GAIN20_MAX)}%\n"
