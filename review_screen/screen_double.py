...
第一层（趋势基础）：8个条件过滤（全部通过才进入第二层）
  ...
  条件3：均线方向（三选一）
    ...
    【加速】MA10/MA20/MA60 短期方向向上 且 当日涨幅>5% 且 当日换手≥5% 且 MA5短期方向向上
    【赢家】MA5>MA10>MA20>MA60（空间有序）且方向全部向上，且当日收盘>MA5 且当日涨幅>2%
  条件4：收盘价 > MA10
  条件5：DIF上涨 且 DIF>0 且 DEA>0（多头排列，非金叉）
  条件6：5日涨幅 > 5%（正常），或（>2% 且近5日≥3天收盘>MA5）（宽松），或（>1% 且当日>5% 且收盘>MA5 且MA5>MA20 且MA10>MA20）（ACCEL放量）
  条件7：当日换手率 > 0（停牌过滤）
  条件8：当日跌幅 < -3% 排除（跌幅过滤）
  条件9：5日均换手率 ≥ 门槛值（≥500亿≥1%，≥50亿≥3%，≥30亿≥5%，≥20亿≥8%，＜20亿≥10%）
...
L2_GAIN_COOL_MIN = 2
L2_GAIN_COOL_MAX = 5
L2_GAIN_ACCEL_MIN = 1  # ACCEL放量路径门槛（1% < 5日涨幅 <= 2% 属于"偏弱ACCEL"）

# ── 可配置参数（CLI覆盖）────────────────────────────────────
_CLI_MODE = "winner"          # "normal" | "accelerated" | "winner"
_CLI_GAIN20_MIN = None        # None = 不设限
_CLI_TURNOVER_MIN = None      # None = 使用市值规则
_CLI_NO_HOT = False           # True = 不区分热点板块
_CLI_TREND_LEVEL = 2          # 0=无过滤 1=MA20向上+ret20>=8% 2=P1+ret20>=12%+close<MA20*1.1+均线发散

def set_cli_params(mode=None, gain20=None, turnover=None, no_hot=False, trend_level=None):
    global _CLI_MODE, _CLI_GAIN20_MIN, _CLI_TURNOVER_MIN, _CLI_NO_HOT, _CLI_TREND_LEVEL
    if mode is not None:
        _CLI_MODE = mode
    ...
    if trend_level is not None:
        _CLI_TREND_LEVEL = int(trend_level)
    _CLI_NO_HOT = no_hot
...
HELP_L1_GAIN_RULE = (
    f"5日涨幅 > {L1_GAIN_STRICT:.0f}%，或（> {L1_GAIN_RELAXED:.0f}% 且近5日≥{L1_RELAX_MA5_DAYS}天收盘>MA5），"
    f"或（>1% 且当日>5% 且收盘>MA5 且MA5>MA20 且MA10>MA20）"
)
HELP_L2_GAIN_RULES = [
    f"8%≤涨幅≤15%：+15分（强）",
...
def calc_macd(closes, fast=12, slow=26, signal=9):
    ...
    dif: 今天（前 n 个数据）的 DIF
    dif_series: [第signal天DIF, ..., 第n-1天DIF]（含今天）
    dea: 基于 dif_series 计算（已含今天的时间对齐）
    """
    n = len(closes)
    ...
    dif = ef - es  # 今天 DIF（用整个数组）
    dif_series = []
    # FIX: range(signal, n) → range(signal, n+1)
    # 修复 off-by-one：dif_series 需包含今天（第 n-1 天），使 dea 与 dif 时间对齐
    for i in range(signal, n+1):
...
def debug_base(code, signal_date):
    ...
    # ── 短期均线方向（2-bar: MA(t) vs MA(t-1)，用于ACCEL路径）─────────
    def ma_short(closes, period, t):
        if t < period: return 0
        now = np.mean(closes[t-period+1:t+1])
        prev = np.mean(closes[t-period:t])
        return 1 if now > prev * 1.001 else (-1 if now < prev * 0.999 else 0)
    ma5_now = np.mean(closes[T_pos-4:T_pos+1])
    ma5_prev = np.mean(closes[T_pos-5:T_pos])
    ma5_short_up = ma5_now > ma5_prev * 1.001
    d10_st = ma_short(closes, 10, T_pos)
    d20_st = ma_short(closes, 20, T_pos)
    d60_st = ma_short(closes, 60, T_pos) if T_pos >= 60 else -1
    accelerated_path = (d10_st == 1 and d20_st == 1 and d60_st == 1
                       and gain_today > 5.0
                       and turnover_today >= 5.0
                       and ma5_short_up)
    # 赢家路径：MA空间有序 + 4线方向向上 + 当日收盘>MA5 + 当日涨幅>2%
    ...
    detail_winner = (f"MA5({ma5:.2f})>MA10({ma10:.2f})={"✓" if ma5>ma10 else "✗"}, "
        ...
        f"当日收盘({close_T:.2f})>MA5({ma5:.2f})={"✓" if close_T>ma5 else "✗"}, "
        f"当日涨幅({gain_today:.1f}%)>2%){"✓" if gain_today>2.0 else "✗"}")
    detail_normal = (f"方向: d5={dir_label(d5)} d10={dir_label(d10)} d20={dir_label(d20)} d60={dir_label(d60)}")
    detail_accel = (f"短期方向(2-bar) d10_st={dir_label(d10_st)} d20_st={dir_label(d20_st)} d60_st={dir_label(d60_st)}, "
        f"当日涨幅={gain_today:+.1f}%, 当日换手={turnover_today:.1f}%（门槛≥5%）, "
        f"MA5短期={'↑' if ma5_short_up else '↓'}")
    if mode == "winner":
    ...
    # 条件8: 当日跌幅过滤（跌幅>3%直接排除）
    c8_pass = gain_today >= -3.0
    results.append(("条件8 当日跌幅过滤", c8_pass, f"当日涨幅={gain_today:+.2f}%（需≥-3%）={"✓" if c8_pass else "✗"}"))

    if not c3_pass or not c2_pass or not c7_pass or not c8_pass:
    ...
    if macd is None:
        ...
        # FIX: 重新计算 DIF_y（时间对齐）
        # dif = calc_macd 的 dif = 今天 DIF（基于整个 closes）；dif_y = 昨天 DIF（基于 closes[:T_pos]）
        # 原先 ef_y 用 closes[:T_pos-1] 实际算的是前天，DIF 动量比较错位一天
        ef_y = calc_ema(closes[:T_pos], 12); es_y = calc_ema(closes[:T_pos], 26)
        dif_y = ef_y - es_y
        dif_up = dif > dif_y
        c5_pass = dif_ok and dea_ok and dif_up
        results.append(("条件5 DIF/DEA/DIF↑", c5_pass,
            f"DIF({dif:.3f})>0={"✓" if dif_ok else "✗"}, DEA({dea:.3f})>0={"✓" if dea_ok else "✗"}, DIF↑={"✓" if dif_up else "✗"}(今{'>' if dif_up else '≤'}昨)"))
    ...
    # c6b: 宽松（>2% 且近5日≥3天>MA5）
    c6b = gain5d > L1_GAIN_RELAXED and days_above_ma5 >= L1_RELAX_MA5_DAYS
    # c6c: ACCEL放量（>1% 且当日>5% 且收盘>MA5 且MA5>MA20 且MA10>MA20）
    gain_today_c6 = (close_T / closes[T_pos-1] - 1) * 100 if T_pos >= 1 else -999
    c6c = (gain5d > 1.0
           and gain_today_c6 > 5.0
           and close_T > ma5
           and ma5 > ma20
           and ma10 > ma20)
    c6_pass = c6a or c6b or c6c
    c6c_detail = (f"，ACCEL放量=✓" if c6c else f"，ACCEL放量=✗("
                   f"gain5d={gain5d:.2f}>1%, gain_today={gain_today_c6:.2f}>5%, "
                   f"close({close_T:.2f})>MA5({ma5:.2f}), MA5>{MA20}({ma20:.2f}), MA10>{MA20}({ma20:.2f})"
                   f"={"✓" if (ma5>ma20 and ma10>ma20) else "✗"})")
    results.append(("条件6 5日涨幅", c6_pass,
        f"5日涨={gain5d:+.2f}%（需>{L1_GAIN_STRICT}%），宽松(c6b)={c6b}（>{L1_GAIN_RELAXED}% 且≥{L1_RELAX_MA5_DAYS}天>MA5），当前>MA5天数={days_above_ma5}{c6c_detail}"))
    ...
    c9_pass = avg_turnover_5 >= threshold
    results.append((f"条件9 均换手≥{threshold}%", c9_pass,
        f"5日均换手={avg_turnover_5:.2f}%（{rule}）={"✓" if c9_pass else "✗"}"))
...
def check_base(code, signal_date):
    ...
    条件3 均线方向模式（通过全局_CLI_MODE控制）：
      ...
      accelerated→ MA10/MA20/MA60 短期方向向上 且 当日涨幅>5% 且 当日换手≥5% 且 MA5短期方向向上
    ...
    # 【正常】MA5/MA10/MA20/MA60 方向全部向上（5日均值 > 5日前均值×1.001）
    normal_path = (d5==1 and d10==1 and d20==1 and d60==1)
    # 【加速】MA10/MA20/MA60 短期方向向上 且 当日涨幅>5% 且 当日换手≥5% 且 MA5短期方向向上（2-bar对比：MA(t) vs MA(t-1)）
    gain_today = (close_T / closes[T_pos-1] - 1) * 100 if T_pos >= 1 else 0
    turnover_today = turnovers[T_pos] if T_pos < len(turnovers) else 0
    # ── 停牌过滤：当日换手率=0视为停牌 ─────────────────────
    ...
    # FIX: dif = 今日 DIF（calc_macd 用整个 closes）；dif_y = 昨日 DIF（用 closes[:T_pos]）
    # 原先 ef_y 用 closes[:T_pos-1] 实际是前天，DIF 动量比较错位一天
    ef_y = calc_ema(closes[:T_pos], 12); es_y = calc_ema(closes[:T_pos], 26)
    dif_y = ef_y - es_y
    ...
    #   条件C（ACCEL放量）：5日涨幅 > 1% 且 当日涨幅 > 5% 且 收盘>MA5 且 MA5>MA20 且 MA10>MA20
    gain5d = (close_T / closes[T_pos-5] - 1) * 100
    days_above_ma5 = 0
    for i in range(max(0, T_pos-4), T_pos+1):
        ma5_i = calc_ma(closes[:i+1], 5)
    ...
    # ── 趋势过滤（_CLI_TREND_LEVEL）────────────────────────
    # P0: 无额外过滤
    # P1: MA20向上 + ret20 >= 8%
    # P2: MA20向上 + ret20 >= 12% + close < MA20*1.10 + 均线发散度约束（MA5>MA10>MA20，间距>1%）
    level = _CLI_TREND_LEVEL
    if level >= 1:
        # MA20 方向（需至少25根K线，window 足够）
        ma20_now = calc_ma(closes, 20)
        ma20_5ago = calc_ma(closes[:-5], 20) if T_pos >= 5 else None
        if ma20_now is not None and ma20_5ago is not None:
            if not (ma20_now > ma20_5ago * 1.001):
                return None
        elif ma20_now is None or ma20_5ago is None:
            return None   # 数据不足，无法判断方向
        # P1 也要求 ret20 >= 8%
        if ret20 is not None and ret20 < 8.0:
            return None
    if level >= 2:
        if ret20 is None:
            return None   # 数据不足，无法判断20日涨幅
        if ret20 < 12.0:
            return None   # 20日涨幅需 >= 12%
        # 价格不能涨太多：收盘价 < MA20 * 1.10（距MA20偏离<10%）
        if close_T >= ma20 * 1.10:
            return None
        # 均线发散度：MA5 > MA10 > MA20 且三者间距均 > 1%
        ma5_10_spread = (ma5 - ma10) / ma10 * 100
        ma10_20_spread = (ma10 - ma20) / ma20 * 100
        if not (ma5 > ma10 > ma20 and ma5_10_spread > 1.0 and ma10_20_spread > 1.0):
            return None
...
def score_stock(sig, hot_sectors):
    ...
    # ── 条件5：5日涨幅健康 ──────────────────────────
    # ACCEL放量路径通过的股票（gain5d 1%~2%），即使"偏弱"也不拒绝
    is_accel_gain = (L2_GAIN_ACCEL_MIN < gain <= L2_GAIN_COOL_MIN)
    if L2_GAIN_STRONG_MIN <= gain <= L2_GAIN_STRONG_MAX:
        ...
        # 1% < 涨幅 <= 2%：ACCEL放量路径，不拒绝但降分
        score += 1
        reasons.append(f"5日涨={gain:.1f}%(ACCEL偏弱,+1)")
    else:
...
def screen_strategy(target_date, top_n=20, single_code=None):
    ...
    # ── 输出文件名：根据模式加后缀 ────────────────────────
    mode_suffix = f"_{_CLI_MODE}" if _CLI_MODE != "normal" else ""
    output_basename = f"screen_double{mode_suffix}_{target_date}"
    ...
    mode_label = {"normal": "", "accelerated": "(ACCEL)", "winner": "(WINNER)"}.get(_CLI_MODE, "")
    txt_lines = [
        f"📊 screen_double{mode_label} {target_date}（第一层 {len(base_signals)} / 第二层 {len(final_signals)} 只）",
        "=" * 120, _header_row(), "-" * 120,
    ]
...
【第一层：趋势基础条件】（全部通过才进入第二层）
  ...
  条件3：均线方向（三选一）
    【正常】MA5/MA10/MA20/MA60 方向全部向上（5日均值 > 5日前均值×1.001）
    【加速】MA10/MA20/MA60 短期方向向上 且 当日涨幅>5% 且 当日换手≥5% 且 MA5短期方向向上
    【赢家】MA5>MA10>MA20>MA60（空间有序）且方向全部向上，且当日收盘>MA5 且当日涨幅>2%（更严格的空间约束）
  条件4：收盘价 > MA10
  条件5：DIF上涨 且 DIF>0 且 DEA>0（多头排列，非金叉）
  条件6：{HELP_L1_GAIN_RULE}
  条件7：当日换手率 > 0（停牌过滤）
  条件8：当日跌幅 < -3% 排除（跌幅过滤）
  条件9：5日均换手率 ≥ 门槛值（默认按市值规则，或通过 --turnover 参数覆盖）
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top-n", type=int, default=500, help="最多输出前N只（默认500）")
    parser.add_argument("--code", default=None, help="单只股票代码，只分析指定股票")
    parser.add_argument("--mode", default="winner",
                        choices=["normal", "accelerated", "winner"],
                        help="条件3均线方向模式：normal/accelerated/winner(默认)")
    parser.add_argument("--gain20", type=float, default=None,
                        help="20日涨幅最低门槛，单位：百分比，默认不设限")
    parser.add_argument("--turnover", type=float, default=None,
                        help="5日均换手率最低门槛，单位：百分比，默认按市值规则")
    ...
    parser.add_argument("--trend-level", type=int, default=2,
                        help="趋势过滤严格度（默认2）\n"
                             "  0=无额外过滤（仅Layer1基础条件）\n"
                             "  1=MA20方向向上 + 近20日涨幅>=8%%\n"
                             "  2=P1 + 近20日涨幅>=12%% + 收盘价<MA20*1.10 + MA5/10/20发散度>1%%")
    ...
    set_cli_params(mode=args.mode, gain20=args.gain20, turnover=args.turnover,
                     no_hot=args.no_hot, trend_level=args.trend_level)
